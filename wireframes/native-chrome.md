...
| Drag region | header | `data-tauri-drag-region` on `.app-header`. Buttons, inputs, toggles are `no-drag`. Left padding ~80px so content clears the lights. Browser: no extra padding. |
