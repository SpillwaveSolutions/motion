...
| Preview body | region | `aria-label` **AI preview**. Raw markdown, mono, scroll. The document is not mutated until Replace / Insert below. |
...
| Try again | button | Re-runs the same instruction. Visible on preview and error. |
| Discard | button | Closes the panel, applies nothing. Escape does the same while the panel is open. |
| Working | status | Copy **Asking AI…**. `aria-busy`. Discard still works (cancels). |
| Error | alert | The failure message lives in the panel. No `window.alert`. |
...
- **Working**: Asking AI…. Editor not editable.
...
- [ ] Failure copy is in the panel (no alert, no uncaught exception).
- [ ] Markdown mode: Refine preview + Replace still work.

## Notes
- Phase 1 transport is `callLLMFromUI` (`POST /api/llm` / `run_llm_cli`). No Anthropic SDK, no sidecar.
- Context is packed by `buildAiContext` (title, surrounding text, selection, prior accepted ops).
